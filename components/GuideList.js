/** @jsx jsx */
import { jsx, Heading, Text, Box, Flex, Grid, Container, Link as ThemeLink } from 'theme-ui';
import Link from 'next/link';
import { Icon } from '@makerdao/dai-ui-icons';

const GuideList = ({ guides, title, columns = [2, 4], path }) => {
  return (
    <Container>
      <Flex>
        <Box>
          <Flex sx={{ mb: 4 }}>
            <Flex>
              <Heading sx={{ textAlign: 'center' }}>{title}</Heading>
            </Flex>
            <Link href={'/guides'} passHref>
              <Flex sx={{ ml: 3, mb: 0, alignItems: 'center' }}>
                <Icon sx={{ mr: 2 }} color="primary" name={'arrow_right'}></Icon>
                <ThemeLink sx={{ color: 'text', cursor: 'pointer' }}>View All</ThemeLink>
              </Flex>
            </Link>
          </Flex>

          <Grid columns={columns}>
            {guides.map(
              ({
                data: {
                  frontmatter: { title, description, slug, parent },
                },
              }) => {
                return (
                  <Flex key={slug} sx={{ flexDirection: 'column' }}>
                    <Flex sx={{ flexDirection: 'column', mb: 2 }}>
                      <Text variant="caps" sx={{ color: 'primary' }}>
                        {parent}
                      </Text>
                    </Flex>

                    <Link key={title} href={`/${path}/${slug}/`}>
                      <Flex sx={{ flexDirection: 'column' }}>
                        <Heading as="a" sx={{ cursor: 'pointer', mb: 2 }}>
                          {title}
                        </Heading>
                        <Text>{description}</Text>
                      </Flex>
                    </Link>
                  </Flex>
                );
              }
            )}
          </Grid>
        </Box>
      </Flex>
    </Container>
  );
};

export default GuideList;
